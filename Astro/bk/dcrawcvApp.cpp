...
DEFINE_string(f, "noflat", "flat frame.");
...
//	cout << O.cols << " " << O.rows << " " << O.channels() << " " << O.type() << endl;
	vector< Mat1d > vvO;
	split(O, vvO);
	resize(vvO[1], vvO[1], Size(), 0.25, 0.25);
	Mat L;
	cv::Laplacian(vvO[1], L, vvO[1].type());
	L = -1 * L - 5000;
...
//複数枚合成
	...
	if(FLAGS_S != 1) {
		double factor = FLAGS_S;
		if(FLAGS_S < 0.0) { factor = 1.0 / abs(FLAGS_S); }
		cv::resize(O, O, Size(), factor, factor);
	}
