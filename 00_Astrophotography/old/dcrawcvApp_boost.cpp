...
//	cout << maxValue << endl;
	...
	R = cv::min(rGain*R,maxValue);
	B = cv::min(bGain*B,maxValue);
...
//	int rotation = 0;
	...
	string wbArgs;
	string backgroundArgs;

	options_description opt("Options");

	opt.add_options()
						("input,i",	value<string>(&input), "input")

						("flat,f",	value<string>(&flat), "Use flat frame")
						("gain,g",	value<float>(&gain), "Gain")
						("WB,W",	value<string>(&wbArgs), "White balance rGain,bGain")
						("bg",	value<string>(&backgroundArgs), "Background r,g,b")
						("Flat,F",	value<string>(&input), "Create flat frame.")
						("smooth,s",	value<float>(&sigma), "Smoothing sigma")
						("scale,S", 	value<float>(&scale), "Scaling factor ")
//						("rotate,R",	value<int>(&rotation), "Rotation 0:none/1:+90/2:+180/3+270")
						;
